...
```
#We can see that Survived, Age and Fare consist of missing values.
...
```
# Since mean and median are very far and median is effective as comapred to mean, replace missing fare with median instead of mena
...
#Pclass3_median <- titanic_mainData %>% filter(Pclass == 3 & Embarked == "S")  %>% summary(titanic_mainData)
#Summary of missing values
colSums(is.na(titanic_mainData) | titanic_mainData=='')


```
# Note Fare is replaced with value 8.05 and no missing values
2.2 Fix the missing value for Embarked.
```{r}
filter(titanic_mainData, is.na(Embarked) | Embarked=='')
```
The missing embarked female belong to cabin B28, not sure why having same ticket, lets fix this.
...
glm.fit <-step(glm.fit)
```
#Verify the coeeficient of best model and test the p-value

```{r}
summary(glm.fit)
```

#Test the variable inflation factor for model
...
1.The std Error are in resonable range
